...
const session = {};
const users = [];
// massages.push({name : session[cookies.session]['name'] , message : massage});
http
        ...
        if (req.url === "/") {
          ...
          // 세션에 id가 똑같은게 없다면 세션에 아이디를 추가해준다.
            ...
            res.writeHead(200, {
              ...
              "Set-Cookie": `session=${uniqueInt}; Expires=${expires.toGMTString()}; Path=/`,
            });
          ...
          if (cookies.session && loginAction(id, pw)) {
